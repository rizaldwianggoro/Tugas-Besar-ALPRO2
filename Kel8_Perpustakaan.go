...
func editPeminjaman(idBuku string, tanggalBaru time.Time) bool {
	for i := 0; i < jumlahPeminjaman; i++ {
		if daftarPeminjaman[i].IDBuku == idBuku {
			fmt.Printf("Mengubah tanggal pinjam buku dengan ID %s\n", idBuku)
			daftarPeminjaman[i].TanggalPinjam = tanggalBaru
			return true
		}
	}
	return false
...
// Fungsi untuk menghapus data peminjaman buku
...
func main() {
	...
	for {
		...
		fmt.Println("6. Edit Buku Pinjaman")
		fmt.Println("7. Hapus Buku Pinjaman")
		fmt.Println("8. Kembalikan Buku")
		fmt.Println("9. Tampilkan Buku Dipinjam")
		fmt.Println("10. Tampilkan 5 Buku Terfavorit")
		fmt.Println("11. Urutkan Buku")
		...
		case 5:
			...
			pinjamBuku(idBuku)
		case 6:
			fmt.Print("Masukkan ID Buku yang ingin diedit peminjamannya: ")
			idBuku, _ := reader.ReadString('\n')
			idBuku = strings.TrimSpace(idBuku)
			fmt.Print("Masukkan tanggal baru (dd-mm-yyyy): ")
			tanggalBaruStr, _ := reader.ReadString('\n')
			tanggalBaruStr = strings.TrimSpace(tanggalBaruStr)
			tanggalBaru, err := time.Parse("02-01-2006", tanggalBaruStr)
			if err != nil {
				fmt.Println("Format tanggal salah.")
				break
			}
			if editPeminjaman(idBuku, tanggalBaru) {
				fmt.Println("Data peminjaman berhasil diedit.")
			} else {
				fmt.Println("Data peminjaman tidak ditemukan.")
			}
		case 7:
			fmt.Print("Masukkan ID Buku yang ingin dihapus peminjamannya: ")
			idBuku, _ := reader.ReadString('\n')
			idBuku = strings.TrimSpace(idBuku)
			if hapusPeminjaman(idBuku) {
